...
- Решение compose-in-vm (см. docs/decisions/compose-in-vm.md) упрощает миграцию:
...
1. **Smoke — ПРОЙДЕН** (2026-06-12, container 1.0.0, macOS 15.6.1):
   ...
   - `container build` ubuntu 24.04 + systemd (Dockerfile из
     docs/container-machine.md → experiments/apple-container/Dockerfile) — ок;
...
2. **Главный тест — docker внутри machine — ПРОЙДЕН** (2026-06-12):
   - docker-ce запечён в образ (см. experiments/apple-container/Dockerfile:
...
Согласуется с `docs/decisions/compose-in-vm.md` (golden pg data запекать слоем образа).
